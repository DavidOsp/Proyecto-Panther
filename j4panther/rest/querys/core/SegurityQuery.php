 ...
 */
define("INTSERT_USER", "INSERT INTO j4user(user_name,password,keyAPI,roles) VALUES(?,?,?,?);");
define("UPDATE_USER", "UPDATE j4user SET  password = ?, keyAPI = ?, roles = ? WHERE id=? ;");
define("SELECT_USER", "SELECT password,user_name,keyAPI,roles FROM j4user WHERE user like ?");
define("VERIFY_KEYAPI", "SELECT COUNT(user_name) FROM j4user WHERE keyAPI=?");
